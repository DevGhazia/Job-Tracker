...
async function clearQueuedApplications() {
  const snapshot = await db.collection("users").doc(DEFAULT_USER_ID).collection("applications").where("status", "==", "Queued").get();
  
  if (snapshot.empty) {
    console.log("No queued applications to remove.");
    return;
  }

  const batch = db.batch();
  snapshot.docs.forEach(doc => {
    console.log(`Deleting queued job: ${doc.data().role} at ${doc.data().company}`);
    batch.delete(doc.ref);
  });

  await batch.commit();
  console.log(`Successfully removed ${snapshot.size} queued application(s) from Firebase.`);
}
